...
(function (EffectUtils) {
    ...
    * obj           对象
    */
    function playScaleEffect(obj, iniScale, toScale) {
        obj.isScaleEffect = true;
        var onComplete1 = function () {
            if (obj != null) {
                var onComplete2 = function () {
                    obj.scaleX = iniScale.x;
                    obj.scaleY = iniScale.y;
                    if (obj.isScaleEffect) {
                        egret.Tween.get(obj).to({ alpha: 1 }, 1000).call(onComplete1, self);
                    }
                    else {
                        egret.Tween.get(obj).to({ alpha: 1 }, 1);
                    }
                };
                obj.alpha = 1;
                egret.Tween.get(obj).to({ scaleX: toScale.x, scaleY: toScale.y, alpha: 0 }, 1000).call(onComplete2, self);
            }
        };
        onComplete1();
    }
    EffectUtils.playScaleEffect = playScaleEffect;
    function removeScaleEffect(obj) {
        obj.isScaleEffect = false;
    }
    EffectUtils.removeScaleEffect = removeScaleEffect;
    /**
    * 显示对象上线浮动特效
    * obj           对象
    * time          浮动时间 毫秒
